...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["tab1-tab1-module"], {
  ...
  function node_modulesRawLoaderDistCjsJsSrcAppTab1Tab1PageHtml(module, __webpack_exports__, __webpack_require__) {
    ...
    __webpack_exports__["default"] = "<ion-header [translucent]=\"true\">\n  <ion-toolbar>\n    <ion-title>\n      Tab 1\n    </ion-title>\n  </ion-toolbar>\n</ion-header>\n\n<ion-content [fullscreen]=\"true\">\n  <ion-header collapse=\"condense\">\n    <ion-toolbar>\n      <ion-title size=\"large\">Tab 1</ion-title>\n    </ion-toolbar>\n  </ion-header>\n  <ion-button (click)=\"startRecording()\"  expand=\"full\" color=\"dark\" >startRecording</ion-button>\n  <ion-button (click)=\"stopRecording()\"  expand=\"full\" color=\"dark\" >stopRecording</ion-button>\n</ion-content>\n";
    /***/
  },
  ...
  function srcAppTab1Tab1PageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var VoiceRecorder = _capacitor_core__WEBPACK_IMPORTED_MODULE_2__["Plugins"].VoiceRecorder;

    var Tab1Page = /*#__PURE__*/function () {
      function Tab1Page() {
        _classCallCheck(this, Tab1Page);

        this.bufferLen = 16384;
      }
      ...
      _createClass(Tab1Page, [{
              ...
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    // });
                    // VoicePlayer.init();
                    // fetch('/assets/test.wav')
                    //     .then((response) => response.arrayBuffer())
                    //     .then((wav) => {
                    //         fdkAac(new Uint8Array(wav), (err, aac) => {
                    //             if (err) {
                     ...
                     */
                    VoiceRecorder.requestAudioRecordingPermission().then(function (result) {
                    ...
                    VoiceRecorder.addListener('audioRecorded', function (data) {
                      console.log(data.recordDataBase64.length);
                      var dataArr = Float32Array.from(atob(data.recordDataBase64).split('').map(function (_char) {
                        return _char.charCodeAt(0);
                      }));
                      console.log(dataArr.length); // const wav = this.encodeWAV(dataArr);
                      // console.log(wav);

                      fetch('/assets/test.wav').then(function (response) {
                        return response.arrayBuffer();
                      }).then(function (wav) {
                        fdkAac(new Uint8Array(wav), function (err, aac) {
                          // fdkAac(new Uint8Array(wav.buffer), (err, aac) => {
                          if (err) {
                            return console.error(err);
                          }

                          var file = 'File' in self ? new File([aac.buffer], 'test.aac', {
                            type: 'audio/aac'
                          }) // Safari does not have File in workers
                          : new Blob([aac.buffer], {
                            type: 'audio/aac'
                          });
                          var url = URL.createObjectURL(file); // Do something with the file or the url, for example:

                          new Audio(url).play();
                        });
                      }); // const file = new File([wav.buffer], 'test.wav', {type: 'audio/wav'});
                      // const url = URL.createObjectURL(file);
                      // Do something with the file or the url, for example:
                      // new Audio(url).play();
                    ...
                    /*          const dataArr = Uint8Array.from(
                                  atob(data.recordDataBase64)
                                      .split('')
                                      .map(char => char.charCodeAt(0)));
                              fdkAac(this.encodeWAV(dataArr), (err, aac) => {
                                  console.log('audio encoded');
                                  if (err) {
                                      return console.error(err);
                                  }
                                  console.log('audio encoded2');
                    ...
                    //         base64: base64Sound
                    //     })
                    //     .then((r3: GenericResponse) => console.log(r3))
                    //     .catch(error => console.log(error));
                    //     // const audioRef = new Audio(`data:audio/aac;base64,${base64Sound}`);
                    //     // audioRef.oncanplaythrough = () => audioRef.play();
                    //     // audioRef.load();
                    //     VoiceRecorder.startRecording({
                    //         source: RecordSource.VOICE_COMMUNICATION
                    //     })
                    //         .then((r2: GenericResponse) => console.log(r2.value))
                    //         .catch(error => console.log(error));
                    // }, 5000);
                    // }
        ...
        key: "writeString",
        value: function writeString(view, offset, str) {
    ...
    Tab1Page.AudioContext = window.AudioContext || window.webkitAudioContext;
    Tab1Page.channels = 1;
    Tab1Page.sampleRate = 44100;
